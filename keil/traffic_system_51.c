```c name=keil/traffic_system_51.c
#include <REG51.H>
...
static void stepper_phase(unsigned char m){
    M1 = (m & 1) ? 1 : 0;
    M2 = (m & 2) ? 1 : 0;
    M3 = (m & 4) ? 1 : 0;
    M4 = (m & 8) ? 1 : 0;
    ...
    switch(ev->code){
        ...
        case KEY_5: case KEY_6: case KEY_7: case KEY_8: case KEY_9:
            if(input_mode && input_digits<4){
                unsigned char d = (unsigned char)(ev->code - KEY_0);
...
void update_display_buffer(unsigned int sec){
    if(sec > 9999){
        display_digits[0] = DASH;
        display_digits[1] = DASH;
        display_digits[2] = DASH;
        display_digits[3] = DASH;
        return;
    }
    {
        unsigned int t = sec;
...
void timer0_isr(void) interrupt 1{
...

    {
        ...
        if(idx==0) DIG1=0;
        else if(idx==1) DIG2=0;
        else if(idx==2) DIG3=0;
        else DIG4=0;
        idx = (unsigned char)((idx + 1) & 3);
    }
}
...
void main(void){
    ...
    last = millis();
    while(1){
        if(keypad_poll(&ev)) apply_key_event(&ev);
